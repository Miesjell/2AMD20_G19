...
class RecipeLoader(DataLoader):
    ...
    def load_data(
        ...
        with self.driver.session() as session:
            ...
            for batch_idx, batch in enumerate(
                tqdm(batches, desc=f"Loading recipes from {source_name}", unit="batch")
            ):
                    ...
                    try:
                        ...
                        recipe = {
                            ...
                            "name": self.clean_text(
                                row_dict.get(
                                    "title", row_dict.get("Name", f"Recipe-{idx}")
                                )
                            ),
                            "description": self.clean_text(
                                row_dict.get("desc", row_dict.get("Description", ""))
                            ),
                            "preparation": self.clean_text(
                                row_dict.get(
                                    "directions", row_dict.get("RecipeInstructions", "")
                                )
                            ),
                            "calories": self._extract_numeric(
                                row, ["calories", "Calories"]
                            ),
                            "fat": self._extract_numeric(row, ["fat", "FatContent"]),
                            "protein": self._extract_numeric(
                                row, ["protein", "ProteinContent"]
                            ),
                            "sodium": self._extract_numeric(
                                row, ["sodium", "SodiumContent"]
                            ),
                        ...
                        if (
                            ...
                            and recipe["ingredients"][0] == "Unknown ingredient"
                        ):
    ...
    def _extract_numeric(
        self, row: pd.Series, possible_columns: List[str]
    ) -> Optional[float]:
        """
        ...
        if not ingredients:
            try:
                for col in row.index:
                    if "ingredient" in str(col).lower() and col not in [
                        ...
                        try:
                            ...
                            if (
                                pd.isna(col_val).all()
                                if hasattr(pd.isna(col_val), "all")
                                else pd.isna(col_val)
                            ):
                                continue
